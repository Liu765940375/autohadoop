...
expect {
        "~]#" {send "sed -i 's/{\%yarn.nodemanager.resource.memory-mb\%}/20480/g' /opt/sparkconf/hadoop/yarn-site.xml;sed -i 's/{\%yarn.nodemanager.resource.cpu-vcores\%}/30/g' /opt/sparkconf/hadoop/yarn-site.xml;sed -i 's/{\%yarn.scheduler.maximum-allocation-mb\%}/20000/g' /opt/sparkconf/hadoop/yarn-site.xml\r"}
}
expect {
