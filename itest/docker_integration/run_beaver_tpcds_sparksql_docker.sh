...
expect {
        "~]#" {send "cp -r /home/$project_name/conf/  /opt/sqlconf;echo \"$master_hostname $master_ip root bdpe123 master\" > /opt/sqlconf/slaves.custom;echo \"$slave1_hostname $slave1_ip root bdpe123 slave\" >> /opt/sqlconf/slaves.custom;sed -i 's/SPARK_VERSION=2.0.0/SPARK_VERSION=2.0.0-hive/g' /opt/sqlconf/env\r"}
}
expect {
...
expect {
        "~]#" {send "rm -rf /etc/yum.repos.d/CentOS-*;cd /home/$project_name/;python bin/runTPCDSonSparkSQL.py deploy_run /opt/sqlconf/\r"}
}
expect {
        "]#" {send "cd /home/$project_name/itest/;source test_utils.sh;service_check;hadoop fs -get /spark/sql/performance/* /opt/Beaver/result/;exit\r"}
}
expect eof
EOF
